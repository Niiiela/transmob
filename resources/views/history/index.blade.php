<x-layout>
    ...
    <div class="max-w-full mt-8 mx-auto bg-white rounded-lg shadow-sm">
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody>
                @foreach ($customer->sent as $customer )
                    <tr class="hover:bg-gray-50 transition-colors border-b">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('tracking.trackings', ['tracker_code' =>$customer->tracker_code]) }}" class="underline">{{ $customer->tracker_code}}</a>
                        </td>
                        <td class="px-6 py-4">{{ $customer->origin}}</td>
                        <td class="px-6 py-4">{{ $customer->destination }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 py-1 rounded-full bg-green-500 text-white">{{ $customer->status }}</span>
                        </td>
                    </tr>
                @endforeach
            </tbody>
